...
**Operator pools.** The pool is the set of anti-Hermitian generators ADAPT-VQE chooses from, and it sets the trade-off between circuit depth and the number of iterations. `fermionic` holds spin-adapted single and double excitations, `qubit` splits them into individual Pauli strings (the shallowest gates, more iterations), `qeb` uses qubit excitations without the Jordan–Wigner Z strings, and `ceo` couples qubit excitations that share an entangling structure for the best accuracy per CNOT. Every pool conserves the particle number, so the search stays in the physical sector.
