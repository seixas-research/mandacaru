...
Carcará uses internally (qubit 0 leftmost).  For the *round-trippable* form of
the Hamiltonian -- the one :class:`~carcara.algorithms.base.VariationalDriver`
reloads with ``load_hamiltonian=`` -- use ``save_hamiltonian=`` instead, which
writes a :class:`~carcara.core.serialization.HamiltonianRecord` in Parquet or
JSON.  Both are capped at :data:`~carcara.core.serialization.MAX_FILE_QUBITS`
