...
r"""The ``mandacaru`` command line.

A thin front end over :class:`~mandacaru.algorithms.Mandacaru`: a geometry
...
qubit budget of the calculation (see :mod:`mandacaru.algorithms.dry_run`), which
...
def solver_options(args) -> dict:
    """Keyword arguments for :class:`~mandacaru.algorithms.Mandacaru`."""
...
def run_full(calc, atoms, args) -> int:
    ...
    silent: :class:`~mandacaru.algorithms.Mandacaru` has no ``verbose`` flag, on
...
def build_backend_command() -> int:
    """``mandacaru --build-backend``: compile the C integral backend.
    ...
    # The MANDACARU_BACKEND policy is about which kernels a *calculation* runs
    ...
    policy = os.environ.get("MANDACARU_BACKEND", "auto").strip().lower() or "auto"
    os.environ["MANDACARU_BACKEND"] = "auto"
    try:
        status = _backend.ensure_backend(build=True, verbose=True)
    finally:
        os.environ["MANDACARU_BACKEND"] = policy
    if status.available:
        ...
        if policy == "numpy":
            print("\nMANDACARU_BACKEND=numpy is set, so calculations in this "
    ...
    print("\nMandacaru will run on the NumPy reference kernels, which give the "
...
def main(argv=None) -> int:
    """``mandacaru`` entry point; returns the process exit code."""
    parser = build_parser()
