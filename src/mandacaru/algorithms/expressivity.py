...
------------------------------------------
The dimension :math:`d` is **not** always :math:`2^N`.  Mandacaru's fermionic
ansätze (:class:`~mandacaru.circuits.ansatz.UCCSD` and the fermionic / QEB / CEO
...
def _resolve_generator(circuit_generator):
    """Return ``(num_parameters, state_fn, n_qubits)`` for a Mandacaru ansatz.

    Accepts any object exposing ``num_parameters`` and ``state(theta)`` -- i.e.
    :class:`~mandacaru.circuits.ansatz.UCCSD`,
    :class:`~mandacaru.algorithms.adapt_vqe.AdaptAnsatz`, or a custom generator with
    the same interface.
    """
...
def sample_pqc_fidelities(circuit_generator, num_samples: int = 1000,
    ...
    the two states with Mandacaru's exact state-vector backend, and returns the
...
def compute_expressibility(circuit, num_qubits: int | None = None,
    ...
    """Measure the expressibility of a Mandacaru ansatz.

    Parameters
    ----------
    circuit : ansatz
        Any generator exposing ``num_parameters`` and ``state(theta)`` (e.g.
        :class:`~mandacaru.circuits.ansatz.UCCSD` or
        :class:`~mandacaru.algorithms.adapt_vqe.AdaptAnsatz`).
...
class ADAPTExpressivityTracker:
    ...
    <mandacaru.algorithms.adapt_vqe.ADAPTVQE.run>`: after each accepted operator it
