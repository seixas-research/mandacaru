...
Where :mod:`mandacaru.basis.sto_ng` builds a *minimal* basis (one contracted
...
As everywhere in Mandacaru, nothing is read from a basis-set table.  The primitive
exponents and contraction coefficients come from the same on-the-fly STO-nG
least-squares fit to Slater-type orbitals used by :mod:`mandacaru.basis.sto_ng`
(:func:`~mandacaru.basis.sto_ng._fit_reference`, rescaled by the subshell's Slater
