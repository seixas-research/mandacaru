...
:mod:`~mandacaru.basis.nao` orbitals: every function is a numerically tabulated
...
all-electron LDA atom (:func:`~mandacaru.basis.atomic_solver.solve_atom`) supplies
...
def radial_tables(functions: list[RadialFunction],
    ...
    """Convert to :class:`~mandacaru.basis.multizeta.RadialTable` (``R = u/r``).

    The table runs from ``r = 0`` (extrapolated) to the wall, where the
    function is exactly zero, so the resulting
    :class:`~mandacaru.basis.multizeta.TabulatedOrbital` has ``r_c`` at the wall.
