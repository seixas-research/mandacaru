...
from ..algorithms.adapt_vqe import ADAPTVQE, ADAPTVQEResult
from ..algorithms.subspace import SubspaceADAPTVQE
...
class VASQE(ADAPTVQE):
    ...
    Subclasses :class:`~mandacaru.algorithms.adapt_vqe.ADAPTVQE`; every ``ADAPTVQE``
    ...
    def run(self, *args, **kwargs) -> VASQEResult:
        ...
        if not isinstance(result, ADAPTVQEResult):       # a dry-run estimate
            return result
...
class SubspaceVASQE(SubspaceADAPTVQE, VASQE):
    ...
    Combines :class:`~mandacaru.algorithms.subspace.SubspaceADAPTVQE` (one shared
