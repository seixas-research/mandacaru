...
"""Forces in the all-electron HAO basis, and what they are worth.
...
class TestGradientIsExact:
    ...
    def test_h2(self, virtual_orbitals):
        ...
        forces, numerical = analytic_and_numerical(
            atoms, {"name": "HAO", "virtual_orbitals": virtual_orbitals},
            grid, 0.35)
    ...
    def test_lih(self):
        ...
        forces, numerical = analytic_and_numerical(atoms, "HAO", grid, 0.35)
        scale = np.abs(numerical).max()
        assert np.abs(forces - numerical).max() < 1e-3 * scale
...
class TestH2:
    ...
    def stretched(self):
        return forces_of("H2", 1.00, "HAO", cell=7.0)[0]
...
class TestLiHCoreArtifact:
    def test_the_net_force_is_enormous(self):
        ...
        forces, calc = forces_of("LiH", 2.19265, "HAO")
