...
_TOTAL_ENERGY_KWARGS = {
    "vqe": dict(h=0.40, optimizer=LBFGS),
    "adapt-vqe": dict(h=0.40, optimizer=LBFGS,
...
class TestConstruction:
    ...
    def test_dimension_and_bands(self, method):
        ...
        assert d.n_bands == 1                       # one HAO 1s per cell
...
class TestTwoDimensional:
    ...
    def square(self):
        atoms = Atoms("H", positions=[[0.0, 0.0, 0.0]],
                      ...
                      pbc=[True, True, False])
        return BlochCalculator(atoms, basis="HAO", n_cells=2, n_images=3, h=0.40)
