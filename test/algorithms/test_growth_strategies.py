...
#: against `LBFGS` above there is nothing to remove, which is its own
#: measurement (see `TestPruning`).
LOOSE = Optimizer(method="L-BFGS", maxiter=2000, tol=1e-8)
...
def run(atoms, optimizer=LBFGS, max_iterations=40, **options):
    work = atoms.copy()
...
class TestOptions:
    ...
    def test_only_booleans(self, option):
        with pytest.raises(ValueError, match=option):
            Mandacaru(method="adapt-vqe", basis="HAO", **{option: "yes"})
