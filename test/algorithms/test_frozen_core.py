...
"""Frozen-core approximation and the HAO actual-atomic-number change."""
...
# --------------------------------------------------------------------------- #
# Part 1: HAO uses the actual atomic number (no Slater screening).
# --------------------------------------------------------------------------- #

class TestHAOActualAtomicNumber:
    def test_hydrogen_unchanged(self):
        # H 1s Slater charge is already 1.0 == Z, so H is unaffected.
        (orb,) = BasisSet.build("HAO").atom("H")
        assert orb.Z == 1.0
    ...
    def test_lithium_uses_bare_Z(self):
        ...
        orbs = BasisSet.build("HAO").atom("Li")
        assert all(o.Z == 3.0 for o in orbs)
        # sanity: the Slater method still exists (used by NAO) and differs.
        assert HydrogenicAtomicOrbital.slater_effective_charge(3, 1, 0) != 3.0

    def test_carbon_uses_bare_Z(self):
        orbs = BasisSet.build("HAO").atom("C")
