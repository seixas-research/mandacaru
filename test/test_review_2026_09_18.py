...
from carcara.algorithms import ADAPTVQE, SubspaceADAPTVQE
from carcara.core.checkpoint import WavefunctionCheckpoint, fingerprint
from carcara.core.mapping import PauliSum
from carcara.core.serialization import load_hamiltonian, save_hamiltonian
...
class TestF01ResumeEnergyIdentity:
    ...
    def _state(path, hamiltonian):
        ADAPTVQE(hamiltonian, "fermionic", checkpoint=path, max_iterations=0,
                 **DIRECT).run()
    ...
    def test_a_shifted_hamiltonian_is_reported_at_its_own_energy(self, tmp_path):
        ...
        with pytest.warns(RuntimeWarning, match="different Hamiltonian"):
            result = ADAPTVQE(shifted, "fermionic", resume=path,
                              max_iterations=0, **DIRECT).run()
    ...
    def test_the_same_hamiltonian_resumes_without_a_warning(self, tmp_path):
        ...
        with warnings.catch_warnings():
            ...
            result = ADAPTVQE(hamiltonian, "fermionic", resume=path,
                              max_iterations=0, **DIRECT).run()
    ...
    def test_a_warm_start_drops_the_foreign_history(self, tmp_path):
        path = str(tmp_path / "state.json")
        ADAPTVQE(PauliSum({"ZIII": 1.0, "XXII": 0.3}), "fermionic",
                 checkpoint=path, max_iterations=2, **DIRECT).run()
        with pytest.warns(RuntimeWarning, match="history discarded"):
            result = ADAPTVQE(PauliSum({"ZIII": 1.0, "XXII": 0.3, "IIII": 1.0}),
                              "fermionic", resume=path, max_iterations=2,
                              **DIRECT).run()
...
class TestF02ReportingOptionsAreHonest:
    ...
    def test_a_method_without_a_log_keeps_its_trace(self, capsys, tmp_path):
        ...
        atoms = h2()
        atoms.calc = Carcara(method="vqe", basis="FAO", h=0.45)
...
class TestF03SubspaceConvergence:
    def test_a_stationary_state_with_no_budget_is_converged(self):
        solver = SubspaceADAPTVQE(PauliSum({"ZIII": 1.0, "IZII": 0.5}),
                                  "fermionic", num_states=2, max_iterations=0,
...
class TestF05CompletionIsNotAssumed:
    def test_the_exit_hook_does_not_claim_completion(self, tmp_path):
        out = str(tmp_path / "output.txt")
        atoms = h2()
        atoms.calc = Carcara(method="adapt-vqe", basis="FAO", h=0.45,
    ...
    def test_a_failed_write_can_be_retried(self, tmp_path, monkeypatch):
        ...
        import carcara.utils.logging as logging_module
...
class TestF06LogLifetime:
    def test_a_missing_parent_is_created_without_consuming_a_step(self, tmp_path):
        from carcara.utils.logging import AdaptOutputLogger, log_steps
...
class TestF07ParserPreservesData:
    ...
    def written(self, tmp_path):
        ...
        from carcara.utils.logging import AdaptOutputLogger
...
class TestF08OutputPathsAndSnapshots:
    def test_the_default_dump_and_cache_names_differ(self):
        from carcara.core.serialization import resolve_save_path
        from carcara.utils.dumps import HAMILTONIAN_FILE, resolve_dump_path
    ...
    def test_two_outputs_on_one_path_are_refused(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="both resolve to"):
            ADAPTVQE(PauliSum({"ZI": 1.0}), num_particles=(1, 1),
                     n_spatial_orbitals=1, save_hamiltonian=path,
                     verbose_hamiltonian=path)

    def test_an_inspection_dump_is_not_loadable_as_a_cache(self, tmp_path):
        from carcara.utils.dumps import dump_hamiltonian
    ...
    def test_a_failed_write_keeps_the_previous_snapshot(self, tmp_path,
        ...
        import carcara.core.serialization as serialization
        def explode(*args, **kwargs):
            raise OSError("disk full")
...
class TestF09ZeroHamiltonianWidth:
    ...
    def test_terms_that_cancel_keep_their_register(self, tmp_path):
        ...
        save_hamiltonian(path, PauliSum({"ZIII": 1.0, "ZIII": 0.0}),
                         num_particles=(1, 1), n_spatial_orbitals=2)
...
class TestF10ProjectedForcesAreQualified:
    ...
    def logged(self, tmp_path_factory):
        ...
        atoms.calc = Carcara(method="adapt-vqe", basis="FAO", h=0.45, charge=1,
...
class TestF11Provenance:
    def test_the_gradient_units_are_stated(self, tmp_path):
        out = str(tmp_path / "output.txt")
        ADAPTVQE(PauliSum({"ZIII": 1.0, "XXII": 0.2}), "fermionic",
                 output=out, max_iterations=1, **DIRECT).run()
    ...
    def test_a_resumed_run_records_its_lineage(self, tmp_path):
        ...
        ADAPTVQE(hamiltonian, "fermionic", checkpoint=state, max_iterations=1,
                 **DIRECT).run()
        ADAPTVQE(hamiltonian, "fermionic", resume=state, output=out,
                 max_iterations=2, **DIRECT).run()
