# -*- coding: utf-8 -*-
# file: test/test_hao_virtual_orbitals.py
...
"""The HAO basis's ``virtual_orbitals`` option: unoccupied levels on demand.

``basis={"name": "HAO", "virtual_orbitals": k}`` appends the ``k`` lowest
...
from mandacaru.basis import BasisSet, HAOBasisSet
...
class TestBasisConstruction:
    def test_the_default_is_the_historical_minimal_basis(self):
        default, explicit = BasisSet.build("HAO"), BasisSet.build(
            "HAO", virtual_orbitals=0)
    ...
    def test_function_counts(self, symbol, counts):
        for k, expected in counts.items():
            basis = BasisSet.build("HAO", virtual_orbitals=k)
    ...
    def test_a_level_is_a_whole_shell(self):
        ...
        """
        basis = BasisSet.build("HAO", virtual_orbitals=2)      # H: +2s, +2p
    ...
    def test_the_occupied_orbitals_are_unchanged(self):
        ...
        minimal = BasisSet.build("HAO").atom("O", center=[0.1, 0.2, 0.3])
        extended = BasisSet.build("HAO", virtual_orbitals=2).atom(
    ...
    def test_virtual_orbitals_carry_the_bare_nuclear_charge(self):
        """Same convention as the occupied HAO orbitals: no Slater screening."""
        basis = BasisSet.build("HAO", virtual_orbitals=1)
        assert {o.Z for o in basis.atom("O")} == {8.0}

    def test_subshells_lists_occupied_then_virtual(self):
        basis = BasisSet.build("HAO", virtual_orbitals=2)
        assert basis.subshells("C") == [(1, 0), (2, 0), (2, 1),
                                        (3, 0), (3, 1)]

    def test_repr_shows_the_option(self):
        assert repr(BasisSet.build("HAO", virtual_orbitals=2)) == \
            "HAOBasisSet(virtual_orbitals=2)"
...
class TestBasisArgument:
    @pytest.mark.parametrize("spec, n_qubits, per_atom", [
        ("HAO", 4, [("H", 1), ("H", 1)]),
        ({"name": "HAO"}, 4, [("H", 1), ("H", 1)]),
        ({"name": "HAO", "virtual_orbitals": 0}, 4, [("H", 1), ("H", 1)]),
        ({"name": "HAO", "virtual_orbitals": 1}, 8, [("H", 2), ("H", 2)]),
        ({"name": "HAO", "virtual_orbitals": 2}, 20, [("H", 5), ("H", 5)]),
    ])
    ...
    def test_per_element(self):
        ...
        estimate = estimate_qubits(
            water, basis={"O": {"name": "HAO", "virtual_orbitals": 1},
                          "H": "HAO"})
    ...
    def test_the_electron_count_does_not_change(self):
        ...
        plain = estimate_qubits(h2(), basis="HAO")
...
class TestVariationalPayoff:
    def test_a_virtual_level_lowers_the_h2_energy(self):
        ...
        The minimal HAO basis is 2 spatial orbitals with a single double
        ...
        for k in (0, 1):
            ...
            atoms.calc = Mandacaru(method="adapt-vqe",
                                   basis={"name": "HAO", "virtual_orbitals": k},
                                   h=0.30,
                                   pool="fermionic",
                                   optimizer=LBFGS,
                                   max_iterations=40,
...
class TestHAONaming:
    def test_hao_names_exist(self):
        from mandacaru.basis import HAOBasisSet, HydrogenicAtomicOrbital
        from mandacaru.core import MolecularIntegrals, minimal_hao_basis
        assert HydrogenicAtomicOrbital is not None
        assert HAOBasisSet is not None
        assert MolecularIntegrals is not None
        assert minimal_hao_basis is not None

    def test_factory_builds_hao(self):
        from mandacaru.basis import BasisSet, HAOBasisSet
        assert isinstance(BasisSet.build("HAO"), HAOBasisSet)
