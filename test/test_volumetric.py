...
def _solved(atoms, **options):
    ...
    atoms.calc = Mandacaru(method="adapt-vqe", basis=options.pop("basis", "FAO"),
...
def h2():
    """H2 at equilibrium, FAO, Jordan-Wigner."""
...
class TestExcitedStates:
    ...
    def test_subspace_levels_have_their_own_density(self):
        atoms = _molecule("H2", [[0, 0, 0], [0, 0, 0.74]], [6, 6, 6])
        atoms.calc = Mandacaru(method="subspace-vqe", basis="FAO", h=H,
...
class TestContracts:
    ...
    def test_before_a_run(self):
        calc = Mandacaru(method="adapt-vqe", basis="FAO", h=H, trace=False)
