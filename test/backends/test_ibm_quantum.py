...
_H2_RUN = """
    ...
    def _h2():
        ...
        return atoms
    atoms = _h2()
    calc = Mandacaru(method="adapt-vqe", pool="ceo", basis="HAO", h=0.4,
...
class TestDriverConfiguration:
    def test_hardware_requires_shots(self):
        with pytest.raises(ValueError, match="shots > 0"):
            Mandacaru(method="adapt-vqe", pool="ceo", basis="HAO",
                      device="ibm-quantum")
        with pytest.raises(ValueError, match="shots > 0"):
            Mandacaru(method="vqe", basis="HAO", device="ibm_kingston",
                      trace=False)

    def test_ibm_device_configures_the_qiskit_provider(self):
        driver = Mandacaru(method="adapt-vqe", pool="ceo", basis="HAO",
    ...
    def test_exact_qiskit_path_is_unchanged(self):
        ...
        exact = Mandacaru(method="adapt-vqe", pool="ceo", basis="HAO",
