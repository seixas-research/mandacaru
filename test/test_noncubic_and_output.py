...
class TestAdaptOutputProtocol:
    def test_output_file_written_and_parseable(self, h2_hamiltonian, tmp_path):
        ...
        result = adapt.run(max_iterations=6, gradient_tol=1e-6,
        ...
        assert parsed["setup"]["classical_optimizer"] == adapt.optimizer.method
    ...
    def test_selected_operator_flagged_in_pool(self, h2_hamiltonian, tmp_path):
        out = str(tmp_path / "output.txt")
        adapt = AdaptVQE(h2_hamiltonian, "fermionic", num_particles=(1, 1),
                         n_spatial_orbitals=2, profile=False)
        adapt.run(max_iterations=4, gradient_tol=1e-6, output_file=out)
        text = open(out, encoding="utf-8").read()
        assert "<== SELECTED" in text                      # 2/3. flag + gradients
        assert "|grad|=" in text
        assert "expressivity_E:" in text
