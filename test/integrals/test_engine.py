...
class TestUnits:
    ...
    def test_orbital_center_converts_to_bohr(self):
        orb = HydrogenicAtomicOrbital(1, 0, 0, center=[0.0, 0.0, 1.0])  # Angstrom
        assert np.allclose(orb.center, [0.0, 0.0, ANGSTROM_TO_BOHR])
        orb_b = HydrogenicAtomicOrbital(1, 0, 0, center=[0.0, 0.0, 1.0], units="bohr")
...
class TestSlater:
    ...
    def test_effective_charge_values(self, atomic_number, n, l, expected):
        Z = HydrogenicAtomicOrbital.slater_effective_charge(atomic_number, n, l)
        assert np.isclose(Z, expected)

    def test_from_slater_sets_Z(self):
        orb = HydrogenicAtomicOrbital.from_slater(2, 0, 0, atomic_number=3)
    ...
    def test_unoccupied_orbital_raises(self):
        ...
        with pytest.raises(ValueError):
            HydrogenicAtomicOrbital.slater_effective_charge(2, 2, 0)

    def test_invalid_atomic_number_raises(self):
        with pytest.raises(ValueError):
            HydrogenicAtomicOrbital.slater_effective_charge(0, 1, 0)
