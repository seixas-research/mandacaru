...
**Important:** the project is mid-build. Implemented end-to-end: localized bases → real-space integral engine → second-quantized `Fermion` Hamiltonian → fermion-to-qubit mappings → **UCCSD ansatz → VQE** and **ADAPT-VQE** (both exact state-vector solvers, validated on H₂), with **RHF/UHF** for the molecular-orbital basis and split-valence (**6-31G(d)**) bases. Still empty stubs: hardware backends and error mitigation. Before building on a module, check whether it actually has code (`wc -l`), because `plan/roadmap.md` mostly describes intended design and its later phases (backends, mitigation) are not yet written. `pyproject.toml` is the source of truth for the build (currently Python ≥ 3.11).
...
**Potential-energy-surface examples** (`examples/`): `generate_h2_pes.py` / `generate_lih_pes.py` scan a bond length and write RHF dissociation curves (three basis sets, atom-referenced) to `data/*_pes_data.csv`; `plot_pes.py` renders them (shared logic in `pes_utils.py`). H₂ is quantitative; LiH is grid-limited by the unresolved Li 1s core (qualitative only).
...
4. `Wavefunction` (`wavefunction.py`) is a thin facade: it reads geometry (ASE), builds the grid + FAO basis, and delegates all physics to the engine.

**Units convention** (`units.py` is the single source of truth): the numerical core — `evaluate`, the grid coordinate arrays, the C backend, the Poisson solver — always works in **atomic units (Bohr, Hartree)**. The user-facing classes (`Grid`, `FullAtomicOrbital`, `Potentials`, `IntegralEngine`) default to the **frontend units (Ångström, eV)** and convert at their boundary via a `units=`/`energy_units=` argument. The legacy `Wavefunction` facade opts back into atomic units (`units="bohr"`, `energy_units="Ha"`), which is why its public API and tests are unchanged. When adding a user-facing length/energy argument, route it through `units.py` and default to Ångström/eV.
