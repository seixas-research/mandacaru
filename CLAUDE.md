...
- `src/carcara/algorithms/adapt_vqe.py` — `ADAPTVQE`, the adaptive grow-then-reoptimize loop (gradient `⟨ψ|[H,A_i]|ψ⟩` over a pool, warm-started inner VQE) on the exact state-vector backend, with a growable `AdaptAnsatz` and Qiskit-based circuit profiling (`profile_ansatz` → CNOT count + depth in a native `{cx, u}` gate set). All four pools reach FCI on H₂. See `examples/01_ADAPTVQE_H2.py` and `examples/02_ADAPTVQE_LiH.py`.
