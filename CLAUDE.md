...
Recently implemented: `backends/hardware.py` (device registry — `AER_simulator` ideal simulator, `ibm-quantum` reserved/not-yet-runnable) and `utils/logging.py` (the ADAPT `output.txt` protocol, `AdaptOutputLogger`/`parse_output`). `ADAPTVQE` (`algorithms/adapt_vqe.py`) is also an **ASE calculator** (`atoms.calc = ADAPTVQE(pool=..., basis="FAO", mapping=..., gradient=..., device=...)`; `atoms.get_total_energy()` builds the Hamiltonian from the geometry and runs the loop, returning eV) with selectable `gradient` (`"classical"` finite-difference / `"parameter-shift_rule"`).
