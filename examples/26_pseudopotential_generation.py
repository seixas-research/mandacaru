...
``"NCPP-TM"``); the bundled library under ``mandacaru/pseudopotentials/library/ncpp``
holds one for every element up to uranium, and ``basis="NCPP"`` on any driver
uses them.  The ONCVPSP and PAW-LCAO families are generated by the same solver
(``generate_oncv`` / ``generate_paw``) and selected with ``basis="ONCVPSP"`` /
``basis="PAW-LCAO"``; see ``27_pseudopotential_calculations.py``.
