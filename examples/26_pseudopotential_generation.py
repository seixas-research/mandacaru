...
``"NCPP-TM"``); the bundled library under ``carcara/pseudopotentials/library/ncpp``
holds one for every element up to uranium, and ``basis="NCPP"`` on any driver
uses them.  The ONCVPSP and PAW families are generated by the same solver
(``generate_oncv`` / ``generate_paw``) and selected with ``basis="ONCVPSP"`` /
``basis="PAW"``; see ``27_pseudopotential_calculations.py``.
...
from carcara.pseudopotentials import generate_pseudopotential
from carcara.pseudopotentials.generation import check_channel, report
...
print("    atoms.calc = Carcara(method='adapt-vqe',")
print("                         basis='NCPP',            # or 'ONCVPSP', 'PAW'")
print("                         h=0.15)")
