# -*- coding: utf-8 -*-
# file: examples/26_pseudopotential_generation.py
...
Why they exist here
-------------------
Mandacaru integrates on a uniform real-space grid.  The :math:`1s` shell of a
...
The generated potentials are the ``"NCPP"`` family (aliases ``"TM"`` /
``"NCPP-TM"``); its library lives in the ``mandacaru-ncpp`` repository, named
by ``MANDACARU_NCPP_PATH`` (set with ``mandacaru --set-ncpp DIR``), and holds
one dataset for every element up to uranium, loaded by symbol wherever
``basis="NCPP"`` is used.  The ONCVPSP and PAW-LCAO families are generated by
the same solver (``generate_oncv`` / ``generate_paw``) and selected with
``basis="ONCVPSP"`` / ``basis="PAW-LCAO"``; see
``27_pseudopotential_calculations.py``.
...
print()
print("Any driver takes them as a basis name -- the family is the basis:")
print("    atoms.calc = Mandacaru(method='adapt-vqe',")
print("                         basis='NCPP',            # or 'ONCVPSP', 'PAW-LCAO'")
print("                         h=0.15)")
print("    basis={'name': 'NCPP', 'size': 'DZP'}   # the multiple-zeta hierarchy")
print()
print("The mandacaru-ncpp library (MANDACARU_NCPP_PATH) covers every element")
print("with Z < 90 and is loaded by symbol.  See example 20 for what it buys.")
print(RULE)
