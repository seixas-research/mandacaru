...
echo trajectory, Fourier energy spectrum, and resolved excitation peaks.
"""
...
def main(txt: str = "output.txt") -> None:
    """Prepare H2 and report its 3D dipole echoes and Fourier energy spectrum."""
    ...
    # A Cartesian field parallel to the bond uses all three dipole components.
    bond = atoms.positions[1] - atoms.positions[0]
    field = 0.01 * bond / np.linalg.norm(bond)
    nuclear_dipole = (atoms.get_atomic_numbers() @ atoms.positions) * ANGSTROM_TO_BOHR
    potential = electric_dipole_potential(
        position_integrals, field=field, nuclear_dipole=nuclear_dipole,
