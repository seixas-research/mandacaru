...
first few excited states **all at once**, in a single optimization —
{class}`~carcara.algorithms.SubspaceVQE` (fixed ansatz) and
{class}`~carcara.algorithms.SubspaceADAPTVQE` (adaptively grown ansatz).
...
Both drivers are ASE calculators. Attach one, evaluate the energy (which builds the
Hamiltonian and runs the subspace search), and read the full spectrum off
`subspace_result`:
...
{class}`~carcara.algorithms.SubspaceADAPTVQE` grows **one shared adaptive ansatz**
...
- **Excited-state tightness depends on the ansatz.** SSVQE is exact only when the
  ...
  looser excited-state bounds; a richer ansatz — or `SubspaceADAPTVQE` with a
  larger pool — tightens them. Increasing the ground-state weight $w_0$ pins the
  ground state more firmly at the cost of the excited estimates.

A complete, runnable script (both drivers, compared to exact diagonalization) is
