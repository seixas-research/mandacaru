...
the **energy levels** — the ground state *and* excited states — both
{class}`~carcara.algorithms.VQE` and {class}`~carcara.algorithms.ADAPTVQE` expose
an `energy_levels` method built on **variational quantum deflation** (VQD).
...
Attach a VQE calculator, evaluate the ground-state energy once (this builds the
Hamiltonian and configures the solver), then ask for the levels:
...
atoms.calc = VQE(basis="FAO", h=0.20, verbose=False)
...
The same call works on {class}`~carcara.algorithms.ADAPTVQE`, which **grows a
fresh deflated ansatz for each level** — both the pool-screening gradient and the
inner re-optimization carry the penalty term, so the adaptive ansatz builds itself
toward the next excited state:

```python
from carcara.algorithms import ADAPTVQE

atoms.calc = ADAPTVQE(pool="fermionic", basis="FAO", h=0.20, verbose=False)
